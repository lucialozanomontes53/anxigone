...
| `victories` | `Victory` | text, occurredAt |
| `activities` | `Activity` (predefinidas: constantes, no persistidas; personalizadas: persistidas, `isCustom: true`) | title, description, energyLevel, isCustom, createdAt |
| `activities` | `ActivityUsage` | activityId, helpfulness (`mucho｜bastante｜poco｜nada`), completedAt |
| `wellbeing-goals` | `WellbeingGoal` | text, targetDate (opcional), checkIns (`GoalCheckIn[]`: date, metGoal, reflection), createdAt |
...
- **ADR-22 — `testTimeout` global vía `vitest-base.config.ts`**: a medida que la suite de tests creció (más de 60 archivos), tests que encadenan varias interacciones de usuario seguidas (`completeIntake`, formularios con varios pasos) empezaron a superar intermitentemente el timeout por defecto de Vitest (5000ms) al ejecutar la suite completa — no por ningún bug, sino por contención de CPU con tantos archivos en paralelo; el mismo test pasa siempre en solitario. En vez de parchear archivo por archivo según les tocara fallar (whack-a-mole, ya ocurrió en 3 archivos distintos), se sube el timeout globalmente a 15000ms mediante `vitest-base.config.ts` en la raíz del proyecto, cargado automáticamente por el builder (`"runnerConfig": true` en `angular.json`, que autodetecta ese nombre de archivo).
- **ADR-23 — Actividades predefinidas como constantes, no como filas de IndexedDB**: `PRESET_ACTIVITIES` vive en el modelo (`activity.model.ts`) como un array `readonly Activity[]` con ids fijos (`preset-walk`, `preset-music`, ...) y `createdAt: null`; nunca se escriben en el store `activities`, que solo contiene las personalizadas del usuario (`isCustom: true`). `ActivitiesStore.allActivities` las combina en memoria (`[...PRESET_ACTIVITIES, ...customActivities]`). `ActivityUsage.activityId` puede apuntar a un id predefinido o a uno personalizado indistintamente, ya que ambos comparten el mismo espacio de ids. Se añade `ENERGY_LEVEL_LABELS` a `shared/models/energy-level.model.ts` (primera feature que renderiza `EnergyLevel` a la persona usuaria), siguiendo la convención habitual de labels separados de los ids.
...
- **Bloque 3 ✅ (0.9.0)**: `victories` (Registro de Victorias, `/logros`).
- **Bloque 4 ✅ (0.10.0)**: `activities` (Banco Personal de Actividades, `/actividades`) — implementa lo que la Fase 3 original reservaba para `activities`.
- **Bloque 5 ✅ (0.11.0)**: `wellbeing-goals` (Objetivos Emocionales, `/objetivos`).
...
- **0.9.0** (2026-07-12): tercer bloque: `victories` (Registro de Victorias / Mis Logros Emocionales, `/logros`) — quick-add con victorias preestablecidas ("Esperé antes de actuar", "No revisé mensajes", ...) + texto libre, dashboard con victorias de la semana/mes, racha de días consecutivos y total (`countWithinDays`/`computeCurrentStreak`, funciones puras testeadas por separado sin TestBed). `DB_VERSION` 6→7 (`victories`). Enlazada desde la home. También en este bloque: sube el timeout de test global a 15000ms (ver ADR-22) tras detectar que el timeout por defecto de Vitest empezaba a ser insuficiente para algunos tests bajo la carga de la suite completa, ya con más de 60 archivos. 244 tests acumulados, cobertura ≥90%, verificado manualmente en navegador (victorias preestablecidas y libres, dashboard actualizándose, racha, eliminar).
- **0.10.0** (2026-07-12): cuarto bloque: `activities` (Banco Personal de Actividades / "Lo Que Me Funciona", `/actividades`) — 6 actividades predefinidas por nivel de energía (constantes, no persistidas, ver ADR-23) más actividades personalizadas del usuario; tras marcar una como hecha, valoración Mucho/Bastante/Poco/Nada; ranking de efectividad ordenado por puntuación media (`computeEffectivenessRanking`, función pura testeada aparte). Añade `ENERGY_LEVEL_LABELS` a `shared/models/energy-level.model.ts`. `DB_VERSION` 7→8 (`activities`, `activityUsages`). Enlazada desde la home. 263 tests acumulados, cobertura ≥90%, verificado manualmente en navegador (marcar actividad predefinida como hecha, valorar, ranking actualizándose, añadir actividad personalizada).
- **0.11.0** (2026-07-16): quinto bloque: `wellbeing-goals` (Objetivos de Bienestar, `/objetivos`) — objetivos preestablecidos ("Esperar antes de actuar", "Dormir mejor", ...) o de texto libre, con fecha objetivo opcional; check-ins con respuesta Sí/No y reflexión libre, guardados como historial dentro del propio objetivo (`GoalCheckIn[]`, sin store separado); progreso semanal/mensual calculado con `computeCheckInProgress` (función pura testeada aparte, mismo patrón que `countWithinDays` de `victories`). `DB_VERSION` 8→9 (`wellbeingGoals`). Enlazada desde la home. 286 tests acumulados, cobertura ≥90%, verificado manualmente en navegador (añadir objetivo preestablecido y libre, registrar check-in, progreso actualizándose, eliminar objetivo).
